...
for ic=1:length(cutElm)
    locVert = unique(ELEM(cutElm(ic,1)).vertices);
...
if(PARAMS.plotInt)
    ...
    plotFileInt = [PARAMS.plotPath PARAMS.plotFileBase '_int.vtk'];
    ...
    for ic=1:length(cutElm)
        ...
        for is=1:size(ELEM(cutElm(ic,1)).vertices,1)
            [~,locVertId] = intersect(locVert,ELEM(cutElm(ic,1)).vertices(is,:));
            segmentToPlot(num_segment+is,1:2) = vertCount+locVertId;
            normalToPlot(num_segment+is,1:2) = ELEM(cutElm(ic,1)).normals(is,1:2);
        end
        vertCount = vertCount+length(locVert);
        num_segment = num_segment + size(ELEM(cutElm(ic,1)).vertices,1);
    end
    ...
    fprintf(pFilI,'VECTORS normal float\n');
    for ic=1:num_segment
    ...
    %% Point data
    fprintf(pFilI,'POINT_DATA %d\n',num_int_nodes);
    fprintf(pFilI,'VECTORS displacement float\n');
    for ie=1:length(cutElm)
        locNod = [ELEM(cutElm(ie,1)).nodes];
        % Get the shape functions
        xe = [NODE(locNod).X]; ye = [NODE(locNod).Y];
