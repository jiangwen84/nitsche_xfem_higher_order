...
addpath(genpath([absolutPath 'Solver/Benchmark_Examples/cantilevered_beam/Fully_Tied_X/']))
...
PARAMS.Xmin = min(x); PARAMS.Xmax = max(x); PARAMS.xint = 0.33;
PARAMS.Ymin = min(y); PARAMS.Ymax = max(y); PARAMS.yint = 0.33;
...
PARAMS.plotPath = [absolutPath 'Pre-PostProcessor/Results/cantilevered_beam/Fully_Tied/Fully_Tied_X/'];
